...
module.exports = ({ sessionManager, billingManager }) => {
  ...
  router.get("/dokumentasi", (req, res) => {
    res.render("dokumentasi-public", {
      title: "Dokumentasi API - w@pi",
      layout: "layouts/main",
      path: "/dokumentasi",
    });
  });
  ...
  router.post("/create-invoice", async (req, res) => {
    try {
      ...
      try {
        ...
        duitkuResp = await createDuitkuInvoice(
          user,
          amount,
          pm,
          productDesc,
      ...
      try {
        await billingManager.addTransaction(
          api_key,
          duitkuResp.merchantOrderId,
          duitkuResp.reference,
          productDesc,
          amount,
          "pending",
          duitkuResp.paymentUrl || null,
        );
      } catch (err) {
      ...
      return res.json({
        ...
        merchantOrderId: duitkuResp.merchantOrderId,
      });
    } catch (error) {
      console.error(
    ...
    try {
      ...
      if (!merchantOrderId || !paymentMethod)
        return res
          ...
          .json({
            success: false,
            message: "merchantOrderId and paymentMethod required",
          });

      // fetch transaction for this user
      const tx = await billingManager.getTransactionByMerchantOrderId(
        user.api_key,
        merchantOrderId,
      );
      if (!tx)
      ...
      if (tx.status !== "pending")
        return res
          ...
          .json({
            success: false,
            message: "Only pending transactions can change payment method",
          });
      ...
      try {
        ...
        console.error(
          "change-payment-method createDuitkuInvoice failed:",
          err && (err.response ? err.response.data : err.message),
        );
        return res
          .status(502)
      ...
      try {
        await billingManager.updateTransactionInvoice(
          merchantOrderId,
          duitkuResp.reference,
          duitkuResp.paymentUrl || null,
        );
        ...
        console.error(
          "Failed to update transaction invoice:",
          err && err.message,
        );
        return res
          .status(500)
