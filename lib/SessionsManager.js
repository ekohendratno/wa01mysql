...
class SessionManager {
            ...
            try {
                ...
                if (error) {
                    ...
                    fs.rm(sessionPath, { recursive: true, force: true }, (rmError) => {
                        if (rmError) {
                            console.error(`Failed to remove corrupt session: ${key}`, rmError);
                        } else {
        ...
        if (deleteFolder) {
            ...
            if (fs.existsSync(sessionPath)) {
                ...
                try {
                    ...
                    console.error(`Failed to remove session folder: ${sessionPath}`, rmError);
                }
            }
        }
    }
    ...
    async getBalanceSummary(apiKey) {
        ...
        try {
            ...
            console.error('Error retrieving balance summary:', error);
            throw error.isBoom ? error : new Boom('Database error', { statusCode: 500 });
        } finally {
            connection.release();
        }
    ...
    async getMessageCounts(apiKey) {
        ...
        try {
            ...
            console.error('Error retrieving message counts:', error);
            throw error.isBoom ? error : new Boom('Database error', { statusCode: 500 });
        } finally {
            connection.release();
        }
