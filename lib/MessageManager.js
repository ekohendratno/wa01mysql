...
class MessageManager {
  ...
  async getMessages(apiKey, status = "") {
    ...
    try {
      ...
      let query =
        "SELECT * FROM messages WHERE uid = ? AND created_at BETWEEN ? AND ?";
  ...
  async registerMessage(apiKey, deviceKey, messageData) {
    ...
    try {
      ...
      let { isGroup, to, text, role } = messageData;
      let type = "personal";
      if (isGroup) {
        type = "group";
      } else {
      ...
      const [result] = await connection.query(
        "INSERT INTO messages (uid, device_id, type, number, message, role, created_at) " +
          "VALUES (?, ?, ?, ?, ?, ?, ?)",
        [uid, deviceId, type, to, text, role, createdAt]
      );
  ...
  async getMessagesLast(apiKey, limit = 3, days = 7) {
    ...
    try {
      ...
      const query = `
                SELECT id, uid, number, status, message, created_at 
                FROM messages 
