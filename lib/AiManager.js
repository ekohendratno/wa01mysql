...
class AiManager {
  ...
  defaultSettings(uid) {
    return {
      ...
      max_tokens: 600,
      system_prompt:
        "Anda adalah asisten layanan pelanggan. Jawab singkat, sopan, dan hanya berdasarkan knowledge base. Jika tidak tahu, arahkan ke admin/operator.",
  ...
  async testPrompt(uid, prompt) {
    ...
    try {
      const knowledgeContext = await this.buildKnowledgeContext(uid);
  ...
  async buildKnowledgeContext(uid) {
    const [rows] = await this.pool.query(
      "SELECT title, source_type, content, url FROM ai_knowledge_sources WHERE uid = ? AND active = 1 ORDER BY updated_at DESC LIMIT 20",
      [uid],
    );
