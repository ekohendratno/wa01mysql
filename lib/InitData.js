...
class InitData {
  ...
  async createTables(connection) {
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS auth_attempts (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY unique_auth_attempt (ip, identifier),
        INDEX idx_auth_attempts_locked (locked_until)
      )
    `);
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS telegram_bots (
        id INT AUTO_INCREMENT PRIMARY KEY,
        uid INT NOT NULL,
        name VARCHAR(120) NOT NULL DEFAULT 'Telegram Bot',
        bot_token TEXT NOT NULL,
        bot_username VARCHAR(120) DEFAULT NULL,
        status ENUM('inactive','active','error') NOT NULL DEFAULT 'inactive',
        mode ENUM('manual','ai_draft','ai_auto') NOT NULL DEFAULT 'manual',
        bridge_wa_enabled TINYINT(1) NOT NULL DEFAULT 0,
        operator_chat_id VARCHAR(80) DEFAULT NULL,
        last_error TEXT NULL,
        webhook_secret VARCHAR(80) NOT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX idx_telegram_bots_uid (uid),
        INDEX idx_telegram_bots_secret (webhook_secret),
        FOREIGN KEY (uid) REFERENCES users(uid) ON DELETE CASCADE
      )
    `);
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS telegram_messages (
        ...
        uid INT NOT NULL,
        bot_id INT NOT NULL,
        telegram_update_id BIGINT DEFAULT NULL,
        chat_id VARCHAR(80) NOT NULL,
        from_id VARCHAR(80) DEFAULT NULL,
        from_name VARCHAR(180) DEFAULT NULL,
        direction ENUM('in','out') NOT NULL DEFAULT 'in',
        message TEXT NULL,
        status ENUM('received','sent','failed','draft') NOT NULL DEFAULT 'received',
        response TEXT NULL,
        ai_handled TINYINT(1) NOT NULL DEFAULT 0,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NULL DEFAULT NULL,
        INDEX idx_telegram_messages_uid (uid),
        INDEX idx_telegram_messages_bot (bot_id),
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS telegram_contacts (
        ...
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uniq_telegram_contact_phone (uid, bot_id, phone),
        INDEX idx_telegram_contacts_chat (bot_id, chat_id),
        FOREIGN KEY (uid) REFERENCES users(uid) ON DELETE CASCADE,
        FOREIGN KEY (bot_id) REFERENCES telegram_bots(id) ON DELETE CASCADE
      )
    `);
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS telegram_shares (
        ...
        invite_code VARCHAR(40) NOT NULL UNIQUE,
        permission_send TINYINT(1) NOT NULL DEFAULT 1,
        permission_reply TINYINT(1) NOT NULL DEFAULT 1,
        ai_reply_mode ENUM('off','draft','auto') NOT NULL DEFAULT 'off',
        status ENUM('pending','active','revoked') NOT NULL DEFAULT 'pending',
        expires_at DATETIME DEFAULT NULL,
        accepted_at DATETIME DEFAULT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX idx_telegram_shares_owner (owner_uid),
        INDEX idx_telegram_shares_shared (shared_uid),
        INDEX idx_telegram_shares_bot (bot_id),
        FOREIGN KEY (bot_id) REFERENCES telegram_bots(id) ON DELETE CASCADE,
        FOREIGN KEY (owner_uid) REFERENCES users(uid) ON DELETE CASCADE,
        FOREIGN KEY (shared_uid) REFERENCES users(uid) ON DELETE CASCADE
      )
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS opt_ins (
        ...
        system_blocked_at TIMESTAMP NULL DEFAULT NULL,
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS audit_logs (
        ...
        INDEX idx_audit_logs_actor (actor_role, actor_uid),
  ...
  async migrateExistingSchema(connection) {
    ...
    await this.ensureColumn(connection, "opt_ins", "reminder_sent_at", "TIMESTAMP NULL DEFAULT NULL");
    await this.ensureColumn(connection, "opt_ins", "system_blocked_at", "TIMESTAMP NULL DEFAULT NULL");
    await this.ensureColumn(connection, "opt_ins", "block_reason", "VARCHAR(255) DEFAULT NULL");
    ...
    await this.ensureColumn(connection, "messages", "recipient_count", "INT NOT NULL DEFAULT 1");
    ...
    await connection.query(
      `UPDATE messages
       SET recipient_count = GREATEST(1, 1 + LENGTH(COALESCE(number, '')) - LENGTH(REPLACE(COALESCE(number, ''), ',', '')))
       WHERE recipient_count IS NULL OR recipient_count <= 1`,
    );
    ...
    await this.ensureColumn(connection, "telegram_bots", "mode", "ENUM('manual','ai_draft','ai_auto') NOT NULL DEFAULT 'manual'");
    await this.ensureColumn(connection, "telegram_bots", "bridge_wa_enabled", "TINYINT(1) NOT NULL DEFAULT 0");
    await this.ensureColumn(connection, "telegram_bots", "operator_chat_id", "VARCHAR(80) DEFAULT NULL");
    await this.ensureColumn(connection, "telegram_bots", "last_error", "TEXT NULL");
    await this.ensureColumn(connection, "telegram_bots", "webhook_secret", "VARCHAR(80) NOT NULL DEFAULT ''");
    await connection.query("UPDATE telegram_bots SET webhook_secret = UUID() WHERE webhook_secret IS NULL OR webhook_secret = ''");
    await this.ensureColumn(connection, "telegram_messages", "ai_handled", "TINYINT(1) NOT NULL DEFAULT 0");
    await this.ensureColumn(connection, "telegram_messages", "updated_at", "TIMESTAMP NULL DEFAULT NULL");
    await this.ensureColumn(connection, "telegram_contacts", "username", "VARCHAR(120) DEFAULT NULL");
    await this.ensureColumn(connection, "telegram_contacts", "updated_at", "TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
    await this.ensureColumn(connection, "telegram_shares", "permission_send", "TINYINT(1) NOT NULL DEFAULT 1");
    await this.ensureColumn(connection, "telegram_shares", "permission_reply", "TINYINT(1) NOT NULL DEFAULT 1");
    await this.ensureColumn(connection, "telegram_shares", "ai_reply_mode", "ENUM('off','draft','auto') NOT NULL DEFAULT 'off'");
    await this.ensureColumn(connection, "telegram_shares", "expires_at", "DATETIME DEFAULT NULL");
    await this.ensureColumn(connection, "telegram_shares", "accepted_at", "DATETIME DEFAULT NULL");
