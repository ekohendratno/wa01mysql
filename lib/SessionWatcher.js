...
class SessionWatcher {
  constructor(sessionManager, deviceManager, folderSession) {
  ...
  init() {
    ...
    this.watcher.on("unlinkDir", (dirPath) => {
      ...
      console.log(
        `[SessionWatcher] Detected removed session folder: ${key} (will verify in 2s)`
      );
      ...
      setTimeout(async () => {
        try {
          ...
          if (fs.existsSync(dirPath)) {
            console.log(
              `[SessionWatcher] Folder ${key} was restored; ignoring removal.`
            );
            return;
          }
          ...
          if (session) {
            ...
            console.log(
              `[SessionWatcher] In-memory session found for ${key}, removing session object.`
            );
          ...
          if (
            this.deviceManager &&
            typeof this.deviceManager.updateDeviceStatus ===
              "function"
          ) {
            try {
              ...
              console.log(
                `[SessionWatcher] Device ${key} marked as removed in DB.`
              );
            } catch (err) {
              console.warn(
                `[SessionWatcher] Failed to update device status for ${key}:`,
                err && err.message
              );
            }
          }
          ...
          console.error(
            "[SessionWatcher] error handling unlinkDir:",
            err && err.message
          );
        }
      }, 2000);
