...
class BillingManager {
  ...
  async getAdminBalanceUsers() {
    ...
    try {
      const [users] = await connection.query(`
        ...
        FROM users u
        LEFT JOIN balances b ON b.uid = u.uid
  ...
  async sendBalanceToUser(uid, amount, note = "") {
    const connection = await this.pool.getConnection();
    try {
      ...
      if (!Number.isInteger(targetUid) || targetUid <= 0) {
        throw new Error("Pengguna tujuan tidak valid.");
      }
      ...
      const [users] = await connection.query(
        "SELECT uid, name, email, active FROM users WHERE uid = ? LIMIT 1",
        [targetUid],
      );
      if (!users.length) {
        throw new Error("Pengguna tujuan tidak ditemukan.");
      }
      await connection.query(
        `INSERT INTO balances (uid, balance, total_used)
         VALUES (?, 0, 0)
      ...
      await connection.query(
        "UPDATE balances SET balance = balance + ?, updated_at = CURRENT_TIMESTAMP WHERE uid = ?",
        [numericAmount, targetUid],
      );

      const merchantOrderId = `ADMIN-${Date.now()}-${targetUid}`;
      const description = note
        ? `Saldo dari admin - ${note}`
        : "Saldo dari admin";

      await connection.query(
        `INSERT INTO transactions
          (uid, merchantOrderId, reference, description, amount, status, whatIs)
         VALUES (?, ?, ?, ?, ?, 'success', '+')`,
        [
          targetUid,
          merchantOrderId,
      ...
      return {
        success: true,
        message: `Saldo berhasil dikirim ke ${users[0].name || users[0].email}.`,
        merchantOrderId,
      };
    } catch (error) {
      await connection.rollback();
  ...
  async getPackages(includeInactive = false) {
    ...
    try {
      const [packages] = await connection.query(`
        SELECT
          p.*,
          p.life_time AS duration,
          p.limit_daily AS message_limit
        FROM packages p
