...
function assertDuitkuConfig() {
    const required = {
        DUITKU_MERCHANT_CODE: config.merchantCode,
        DUITKU_MERCHANT_KEY: config.merchantKey,
        DUITKU_CALLBACK_URL: config.callbackUrl,
        DUITKU_RETURN_URL: config.returnUrl,
    };
    ...
    if (missing.length > 0) {
        throw new Error(`Konfigurasi Duitku belum lengkap: ${missing.join(', ')}`);
    }
}
