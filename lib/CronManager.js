const cron = require("node-cron");
const moment = require("moment-timezone");

class CronManager {
  constructor(pool, messageManager, sessionManager, billingManager) {
    this.pool = pool;
    this.messageManager = messageManager;
    this.sessionManager = sessionManager;
    this.billingManager = billingManager;
    this.tasks = [];

    // Flags untuk mencegah overlap task
    this.cronProcessingFlags = {
      group: false,
      personal: false,
      bulk: false,
    };

    // Batas pesan harian per device (anti-blokir)
    this.DAILY_MESSAGE_LIMIT = 250;
    this.SAFE_DAILY_PERCENT = Math.min(
      100,
      Math.max(1, Number(process.env.WA_SAFE_DAILY_PERCENT || 90)),
    );

    // Konfigurasi delay (anti-spam/ban)
    this.DELAY_BETWEEN_MESSAGES = { min: 8000, max: 20000 }; // 8–20s antar pesan
    this.DELAY_BETWEEN_SESSIONS = { min: 15000, max: 45000 }; // 15–45s antar sesi

    this.MESSAGES_BEFORE_MICRO_SLEEP = 10;
    this.MICRO_SLEEP = { min: 30000, max: 90000 }; // 30–90 detik
  }

  async initCrons() {
    console.log("✅ Cron Manager initialized with anti-ban strategies.");

    // === CRON UTAMA: Pengiriman pesan ===
    const schedule =
      process.env.CRON_OPERATIONAL_24H === "1" ||
      process.env.CRON_OPERATIONAL_24H === "true"
        ? "*/20 * * * * *" // Tiap 20 detik 24 jam
        : "*/20 * 6-23 * * *"; // Tiap 20 detik 06-23

    const mainTask = cron.schedule(
      schedule,
      async () => {
        const now = moment().tz("Asia/Jakarta");
        const hour = now.hour();
        const is24H =
          process.env.CRON_OPERATIONAL_24H === "1" ||
          process.env.CRON_OPERATIONAL_24H === "true";

        if (!is24H && (hour < 6 || hour >= 24)) {
          console.log(
            `🌙 [Anti-Ban] Skip sending outside operational hours (${hour}:00).`,
          );
          return;
        }

        for (const type of ["group", "personal", "bulk"]) {
          if (this.cronProcessingFlags[type]) {
            console.log(`⚠️ [${type}] already processing, skip this tick.`);
            continue;
          }

          this.cronProcessingFlags[type] = true;
          try {
            console.log(`🚀 Processing [${type}] messages...`);
            await this.processMessagesByType(type);
          } catch (err) {
            console.error(`❌ Error in [${type}] process:`, err.message);
          } finally {
            this.cronProcessingFlags[type] = false;
          }
        }
      },
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(mainTask);

    // === CRON LAIN ===
    const t1 = cron.schedule(
      "0 0 * * *",
      () => this.safeExec(this.decrementDeviceLifeTime, "Decrement life_time"),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t1);
    const t2 = cron.schedule(
      "0 9,15 * * *",
      () => this.safeExec(this.generateDeadlineWarnings, "Deadline warning"),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t2);
    const t3 = cron.schedule(
      "0 * * * *", // Run every hour
      () => this.safeExec(this.deleteOldMessages, "Delete old messages"),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t3);

    const t4 = cron.schedule(
      "*/1 * * * *",
      () =>
        this.safeExec(
          this.removeRemovedDeviceSessions,
          "Remove removed sessions",
        ),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t4);

    // Check pending transactions and mark expired ones as failed (or update if paid)
    const t5 = cron.schedule(
      "*/5 * * * *",
      () =>
        this.safeExec(
          this.checkPendingTransactions,
          "Pending transactions check",
        ),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t5);

    const t6 = cron.schedule(
      "30 3 * * *",
      () =>
        this.safeExec(
          this.cleanupOldDatabaseData,
          "Cleanup old database data",
        ),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t6);

    const t7 = cron.schedule(
      "15 * * * *",
      () =>
        this.safeExec(
          this.processPendingOptIns,
          "Pending Opt-In reminder/block",
        ),
      { timezone: "Asia/Jakarta" },
    );
    this.tasks.push(t7);
  }

  async stop() {
    try {
      this.tasks.forEach((t) => t.stop && t.stop());
      this.tasks = [];
      console.log("✅ Cron Manager stopped.");
    } catch (err) {
      console.error("Failed to stop CronManager:", err && err.message);
    }
  }

  // Wrapper biar setiap cron aman (nggak bikin crash server)
  async safeExec(fn, label) {
    try {
      console.log(`⏳ [Cron] Running task: ${label}`);
      await fn.call(this);
      console.log(`✅ [Cron] Finished task: ${label}`);
    } catch (err) {
      console.error(`❌ [Cron] Error in task ${label}:`, err.message);
    }
  }

  // ====================== CLEANUP ======================

  async deleteOldMessages() {
    const now = moment().tz("Asia/Jakarta");
    const twentyFourHoursAgo = now
      .clone()
      .subtract(24, "hours")
      .format("YYYY-MM-DD HH:mm:ss");

    // Optional: Keep cleaning extremely old failed/pending messages (e.g., 2 days) to avoid accumulation
    const twoDaysAgo = now
      .clone()
      .subtract(48, "hours")
      .format("YYYY-MM-DD HH:mm:ss");

    try {
      const [sent] = await this.pool.query(
        `DELETE FROM messages WHERE status = 'sent' AND created_at < ?`,
        [twentyFourHoursAgo],
      );
      console.log(
        `🧹 Deleted ${sent.affectedRows} sent messages older than 24 hours.`,
      );

      const [others] = await this.pool.query(
        `DELETE FROM messages WHERE status IN ('pending','failed','processing') AND created_at < ?`,
        [twoDaysAgo],
      );
      console.log(
        `🧹 Deleted ${others.affectedRows} stale messages older than 48 hours.`,
      );
    } catch (err) {
      console.error("❌ Cleanup failed:", err.message);
    }
  }

  getCleanupDays(envName, fallbackDays) {
    const value = parseInt(process.env[envName] || "", 10);
    return Number.isFinite(value) && value > 0 ? value : fallbackDays;
  }

  async cleanupOldDatabaseData() {
    const now = moment().tz("Asia/Jakarta");
    const retention = {
      inbox: this.getCleanupDays("CLEANUP_INBOX_DAYS", 30),
      webhookLogs: this.getCleanupDays("CLEANUP_WEBHOOK_LOG_DAYS", 14),
      logs: this.getCleanupDays("CLEANUP_LOG_DAYS", 30),
      pendingTransactions: this.getCleanupDays(
        "CLEANUP_PENDING_TRANSACTION_DAYS",
        90,
      ),
      failedTransactions: this.getCleanupDays(
        "CLEANUP_FAILED_TRANSACTION_DAYS",
        180,
      ),
      pendingShares: this.getCleanupDays("CLEANUP_PENDING_SHARE_DAYS", 7),
      revokedShares: this.getCleanupDays("CLEANUP_REVOKED_SHARE_DAYS", 30),
      optInDeleted: this.getCleanupDays("CLEANUP_OPT_IN_DELETED_DAYS", 90),
      campaigns: this.getCleanupDays("CLEANUP_CAMPAIGN_DAYS", 90),
    };

    const cutoff = (days) =>
      now.clone().subtract(days, "days").format("YYYY-MM-DD HH:mm:ss");

    const deletions = [
      {
        label: `inbox_messages older than ${retention.inbox} days`,
        sql: "DELETE FROM inbox_messages WHERE received_at < ?",
        params: [cutoff(retention.inbox)],
      },
      {
        label: `webhook_logs older than ${retention.webhookLogs} days`,
        sql: "DELETE FROM webhook_logs WHERE created_at < ?",
        params: [cutoff(retention.webhookLogs)],
      },
      {
        label: `logs older than ${retention.logs} days`,
        sql: "DELETE FROM logs WHERE created_at < ?",
        params: [cutoff(retention.logs)],
      },
      {
        label: "expired web sessions",
        sql: "DELETE FROM sessions WHERE expires < UNIX_TIMESTAMP()",
        params: [],
      },
      {
        label: `pending transactions older than ${retention.pendingTransactions} days`,
        sql:
          "DELETE FROM transactions WHERE status = 'pending' AND created_at < ?",
        params: [cutoff(retention.pendingTransactions)],
      },
      {
        label: `failed transactions older than ${retention.failedTransactions} days`,
        sql:
          "DELETE FROM transactions WHERE status = 'failed' AND created_at < ?",
        params: [cutoff(retention.failedTransactions)],
      },
      {
        label: `expired pending device shares older than ${retention.pendingShares} days`,
        sql:
          "DELETE FROM device_shares WHERE status = 'pending' AND (created_at < ? OR (expires_at IS NOT NULL AND expires_at < NOW()))",
        params: [cutoff(retention.pendingShares)],
      },
      {
        label: `revoked device shares older than ${retention.revokedShares} days`,
        sql: "DELETE FROM device_shares WHERE status = 'revoked' AND updated_at < ?",
        params: [cutoff(retention.revokedShares)],
      },
      {
        label: `opt_in_deleted older than ${retention.optInDeleted} days`,
        sql: "DELETE FROM opt_in_deleted WHERE deleted_at < ?",
        params: [cutoff(retention.optInDeleted)],
      },
      {
        label: "orphan groups without active device",
        sql:
          "DELETE g FROM `groups` g LEFT JOIN devices d ON d.device_key = g.device_key WHERE d.id IS NULL",
        params: [],
      },
      {
        label: `completed/failed campaigns older than ${retention.campaigns} days`,
        sql:
          "DELETE FROM campaigns WHERE status IN ('completed','failed','cancelled','canceled') AND created_at < ?",
        params: [cutoff(retention.campaigns)],
      },
    ];

    for (const item of deletions) {
      try {
        const [result] = await this.pool.query(item.sql, item.params);
        console.log(
          `🧹 Cleanup ${item.label}: deleted ${result.affectedRows || 0} rows.`,
        );
      } catch (err) {
        console.error(`❌ Cleanup failed for ${item.label}:`, err.message);
      }
    }
  }

  async getOptInReminderText(userRow) {
    const template = userRow?.template_invitation;
    if (template) {
      return `${template}\n\nPengingat: status persetujuan Anda masih pending. Jika dalam 1x24 jam belum ada balasan SETUJU, sistem akan menghentikan pengiriman notifikasi otomatis ke nomor ini.`;
    }

    return "Halo, ini pengingat persetujuan layanan notifikasi otomatis. Balas SETUJU jika Anda bersedia menerima informasi dan pembaruan dari kami melalui WhatsApp. Jika tidak ada balasan dalam 1x24 jam, sistem akan memblokir opt-in nomor ini secara otomatis. Terima kasih.";
  }

  async processPendingOptIns() {
    const now = moment().tz("Asia/Jakarta");
    const sevenDaysAgo = now.clone().subtract(7, "days").format("YYYY-MM-DD HH:mm:ss");
    const oneDayAgo = now.clone().subtract(24, "hours").format("YYYY-MM-DD HH:mm:ss");
    const currentTime = now.format("YYYY-MM-DD HH:mm:ss");

    const [blockResult] = await this.pool.query(
      `UPDATE opt_ins
       SET status = 'blocked',
           source = 'system_no_response',
           system_blocked_at = ?,
           block_reason = 'Tidak ada balasan 1x24 jam setelah pengingat opt-in',
           updated_at = ?
       WHERE status = 'pending'
         AND reminder_sent_at IS NOT NULL
         AND reminder_sent_at <= ?`,
      [currentTime, currentTime, oneDayAgo],
    );
    if (blockResult.affectedRows) {
      console.log(
        `🚫 [Opt-In] Auto-blocked ${blockResult.affectedRows} pending opt-ins without response after reminder.`,
      );
    }

    const limit = Math.max(1, parseInt(process.env.OPT_IN_REMINDER_BATCH_LIMIT || "50", 10));
    const [rows] = await this.pool.query(
      `SELECT
          oi.id,
          oi.uid,
          oi.device_id,
          oi.number,
          u.api_key,
          u.template_invitation,
          d.device_key,
          d.status AS device_status
       FROM opt_ins oi
       JOIN users u ON u.uid = oi.uid
       JOIN devices d ON d.id = oi.device_id
       WHERE oi.status = 'pending'
         AND oi.reminder_sent_at IS NULL
         AND oi.created_at <= ?
         AND u.active = 1
         AND d.status = 'connected'
       ORDER BY oi.created_at ASC
       LIMIT ?`,
      [sevenDaysAgo, limit],
    );

    let sent = 0;
    for (const row of rows) {
      try {
        const text = await this.getOptInReminderText(row);
        const response = await this.messageManager.sendMessage(
          row.api_key,
          row.device_key,
          row.number,
          text,
          0,
          "opt-in",
        );
        const ok = Boolean(
          response?.status ||
          response?.data?.results?.some((item) => item && item.status),
        );
        if (!ok) {
          console.warn(`⚠️ [Opt-In] Reminder failed for ${row.number}`);
          continue;
        }

        await this.pool.query(
          "UPDATE opt_ins SET reminder_sent_at = ?, updated_at = ? WHERE id = ? AND status = 'pending'",
          [currentTime, currentTime, row.id],
        );
        sent++;
        await this.delay(this.randomDelay(this.DELAY_BETWEEN_MESSAGES));
      } catch (error) {
        console.error(`❌ [Opt-In] Reminder error for ${row.number}:`, error.message);
      }
    }

    if (sent) {
      console.log(`✉️ [Opt-In] Sent ${sent} pending opt-in reminders.`);
    }
  }

  // ====================== SESSION FETCH ======================

  async fetchActiveSessions(limit, offset) {
    const sql = `
      SELECT *
      FROM (
        SELECT u.uid, u.api_key, d.device_key, d.id AS device_id, d.name AS device_name,
               d.phone AS device_phone, d.life_time, 'owner' AS access_type
        FROM users u
        JOIN devices d ON u.uid = d.uid
        WHERE u.active = 1 AND d.status = 'connected'
        UNION ALL
        SELECT u.uid, u.api_key, d.device_key, d.id AS device_id, d.name AS device_name,
               d.phone AS device_phone, d.life_time, 'shared' AS access_type
        FROM device_shares ds
        JOIN users u ON u.uid = ds.shared_uid
        JOIN devices d ON d.id = ds.device_id
        WHERE u.active = 1
          AND ds.status = 'active'
          AND ds.permission_send = 1
          AND d.status = 'connected'
          AND (ds.expires_at IS NULL OR ds.expires_at > NOW())
      ) active_sessions
      ORDER BY device_id ASC, uid ASC
      LIMIT ? OFFSET ?`;

    const [rows] = await this.pool.query(sql, [limit, offset]);
    return rows.map((r) => ({
      uid: r.uid,
      apiKey: r.api_key,
      deviceKey: r.device_key,
      deviceId: r.device_id,
      deviceName: r.device_name,
      devicePhone: r.device_phone,
      deviceLifeTime: r.life_time,
      accessType: r.access_type,
    }));
  }

  // ====================== MESSAGE PROCESS ======================

  async processMessagesByType(type) {
    const SESSION_LIMIT = 10;
    let offset = 0;

    while (true) {
      const sessions = await this.fetchActiveSessions(SESSION_LIMIT, offset);
      if (!sessions.length) break;

      for (const session of sessions) {
        try {
          await this.processMessages(session, type);
        } catch (err) {
          console.error(
            `❌ Error processing ${type} for device ${session.deviceKey}:`,
            err.message,
          );
        }

        await this.delay(this.randomDelay(this.DELAY_BETWEEN_SESSIONS));
      }

      offset += SESSION_LIMIT;
    }
  }

  async processMessages(session, type) {
    if (!session) return;

    const { uid, apiKey, deviceKey, deviceId, accessType } = session;
    const BATCH_LIMIT = 15;
    const now = moment().tz("Asia/Jakarta");
    const today = now.format("YYYY-MM-DD");
    const currentTime = now.format("YYYY-MM-DD HH:mm:ss");
    const optInConfig = await this.getUserOptInConfig(uid, accessType);

    // Ambil limit harian dari DB
    let dailyLimit = this.DAILY_MESSAGE_LIMIT; // default 250
    try {
      const [rows] = await this.pool.query(
        "SELECT limit_daily FROM devices WHERE id=? LIMIT 1",
        [deviceId],
      );
      if (rows.length && rows[0].limit_daily > 0) {
        dailyLimit = rows[0].limit_daily;
      }
    } catch (err) {
      console.warn(
        `⚠️ Gagal ambil limit_daily device ${deviceKey}, pakai default ${this.DAILY_MESSAGE_LIMIT}`,
        err.message,
      );
    }

    // Anti-spam: cek daily limit
    const [dailyCount] = await this.pool.query(
      `SELECT COALESCE(SUM(
          COALESCE(NULLIF(recipient_count, 0), 1 + LENGTH(COALESCE(number, '')) - LENGTH(REPLACE(COALESCE(number, ''), ',', '')))
        ), 0) AS sentToday
       FROM messages
       WHERE device_id = ?
         AND DATE(COALESCE(updated_at, created_at)) = ?
         AND status IN ('sent', 'processing')`,
      [deviceId, today],
    );
    const sentToday = Number(dailyCount?.[0]?.sentToday || 0);
    const safeDailyLimit =
      dailyLimit > 0
        ? Math.max(1, Math.floor((dailyLimit * this.SAFE_DAILY_PERCENT) / 100))
        : dailyLimit;
    const remaining = safeDailyLimit - sentToday;
    if (remaining <= 0) {
      console.log(
        `🚫 [Anti-Ban] Limit reached (${dailyLimit}) for device ${deviceKey}`,
      );
      return;
    }

    // Lock pesan
    const lockLimit = Math.min(BATCH_LIMIT, remaining);
    await this.pool.query(
      `UPDATE messages SET status='processing', updated_at=? 
       WHERE status='pending'
         AND type=?
         AND uid=?
         AND device_id=?
         AND DATE(COALESCE(scheduled_at, created_at)) <= ?
         AND (scheduled_at IS NULL OR scheduled_at <= ?)
         AND COALESCE(NULLIF(recipient_count, 0), 1 + LENGTH(COALESCE(number, '')) - LENGTH(REPLACE(COALESCE(number, ''), ',', ''))) <= ?
       ORDER BY
         CASE
           WHEN response LIKE 'Menunggu Opt-In:%' THEN 1
           ELSE 0
         END ASC,
         COALESCE(scheduled_at, created_at) ASC
       LIMIT ?`,
      [currentTime, type, uid, deviceId, today, currentTime, remaining, lockLimit],
    );

    const [messages] = await this.pool.query(
      `SELECT *
       FROM messages
       WHERE status='processing'
         AND type=?
         AND uid=?
         AND device_id=?
         AND DATE(COALESCE(scheduled_at, created_at)) <= ?
       ORDER BY
         CASE
           WHEN response LIKE 'Menunggu Opt-In:%' THEN 1
           ELSE 0
         END ASC,
         COALESCE(scheduled_at, created_at) ASC`,
      [type, uid, deviceId, today],
    );
    if (!messages.length) return;

    let failedConsecutive = 0;

    for (let i = 0; i < messages.length; i++) {
      const msg = messages[i];
      if (failedConsecutive >= 3) {
        console.warn(`⚠️ Too many failures, pausing for device ${deviceKey}`);
        break;
      }

      let { id, number, message, type: msgType, tags } = msg;
      if (!number || !message) continue;

      // --- OPT-IN CHECK (Extra Safety) ---
      if (!msgType || msgType !== "group") {
        const recipients = number
          .split(",")
          .map((n) => n.trim())
          .filter(Boolean);
        const isBulkRow = msgType === "bulk" || recipients.length > 1;

        if (optInConfig.required) {
          const optInGate = await this.ensureOptInBeforeSending({
            uid,
            apiKey,
            deviceKey,
            deviceId,
            recipients,
            invitationText: optInConfig.invitationText,
            currentTime,
            messageId: id,
          });

          if (optInGate.waiting) {
            console.log(
              `⏳ Message ${id} menunggu Opt-In dari: ${optInGate.waitingRecipients.join(", ")}`,
            );
            continue;
          }

          if (optInGate.blocked) {
            await this.pool.query(
              "UPDATE messages SET status='failed', response=?, updated_at=? WHERE id=?",
              [
                `Opt-In ditolak/blokir: ${optInGate.blockedRecipients.join(", ")}`,
                currentTime,
                id,
              ],
            );
            continue;
          }
        }

        // NOTE:
        // MessageManager sudah melakukan opt-in check per recipient.
        // Jadi untuk bulk, kita tidak boleh cek opt-in "sekali untuk gabungan nomor"
        // karena akan mengubah format nomor dan bikin banyak row jadi salah 'failed'.
        if (isBulkRow) {
          const isAutoInviteEnabled =
            process.env.AUTO_OPT_IN_INVITATION === "true" ||
            process.env.AUTO_OPT_IN_INVITATION === "1";

          if (isAutoInviteEnabled) {
            const maxAutoInvites = parseInt(
              process.env.MAX_AUTO_INVITES_PER_BULK || "3",
              10,
            );
            let autoInvitesSent = 0;

            // Ambil template sekali untuk mengurangi query berulang
            const [userRows] = await this.pool.query(
              "SELECT template_invitation FROM users WHERE uid = ? LIMIT 1",
              [uid],
            );
            let invitationText = userRows[0]?.template_invitation;
            if (!invitationText) {
              invitationText =
                "Halo, ini adalah layanan notifikasi otomatis. Balas pesan ini dengan ketik 'SETUJU' untuk menerima informasi dan pembaruan dari kami secara otomatis melalui WhatsApp. Balas 'STOP' jika ingin berhenti berlangganan. Terima kasih!";
            }

            for (const recipient of recipients) {
              if (autoInvitesSent >= maxAutoInvites) break;
              const check = await this.messageManager.checkOptInStatus(
                uid,
                recipient,
                tags,
              );

              if (check.allowed) continue;
              if (!check.reason.includes("belum melakukan Opt-In")) continue;

              const cleanNumber = recipient.replace(/\D/g, "");
              if (!cleanNumber) continue;

              const [optSearch] = await this.pool.query(
                "SELECT id, source FROM opt_ins WHERE uid = ? AND number = ? LIMIT 1",
                [uid, cleanNumber],
              );

              // Kirim undangan otomatis sekali (bila belum pernah tercatat)
              if (optSearch.length === 0) {
                console.log(
                  `✉️ [Auto-Invitation] Sending opt-in invite to ${recipient}...`,
                );

                // Kirim invitation bypass check (tag: opt-in)
                await this.messageManager.sendMessage(
                  apiKey,
                  deviceKey,
                  recipient,
                  invitationText,
                  0,
                  "opt-in",
                );

                await this.pool.query(
                  "INSERT INTO opt_ins (uid, device_id, number, status, source) VALUES (?, ?, ?, 'pending', 'auto_invitation')",
                  [uid, deviceId, cleanNumber],
                );

                autoInvitesSent++;
                // Jeda antar undangan supaya tidak terlihat seperti spam
                await this.delay(this.randomDelay(this.DELAY_BETWEEN_MESSAGES));
              }
            }
          }
        } else {
          const check = await this.messageManager.checkOptInStatus(
            uid,
            number,
            tags,
          );
          if (!check.allowed) {
            // --- AUTO OPT-IN INVITATION ---
            const isAutoInviteEnabled =
              process.env.AUTO_OPT_IN_INVITATION === "true" ||
              process.env.AUTO_OPT_IN_INVITATION === "1";
            if (
              isAutoInviteEnabled &&
              check.reason.includes("belum melakukan Opt-In")
            ) {
              const cleanNumber = number.replace(/\D/g, "");
              const [optSearch] = await this.pool.query(
                "SELECT id, source FROM opt_ins WHERE uid = ? AND number = ? LIMIT 1",
                [uid, cleanNumber],
              );

              // Jika belum ada di table opt_ins sama sekali, kirim undangan otomatis sekali
              if (optSearch.length === 0) {
                // Ambil template dari user
                const [userRows] = await this.pool.query(
                  "SELECT template_invitation FROM users WHERE uid = ? LIMIT 1",
                  [uid],
                );

                let invitationText = userRows[0]?.template_invitation;

                if (!invitationText) {
                  invitationText =
                    "Halo, ini adalah layanan notifikasi otomatis. Balas pesan ini dengan ketik 'SETUJU' untuk menerima informasi dan pembaruan dari kami secara otomatis melalui WhatsApp. Balas 'STOP' jika ingin berhenti berlangganan. Terima kasih!";
                }

                console.log(
                  `✉️ [Auto-Invitation] Sending opt-in invite to ${number}...`,
                );

                // Kirim invitation bypass check (tag: opt-in)
                await this.messageManager.sendMessage(
                  apiKey,
                  deviceKey,
                  number,
                  invitationText,
                  0,
                  "opt-in",
                );

                // Catat di opt_ins agar tidak kirim ulang (status pending, source auto_invitation)
                await this.pool.query(
                  "INSERT INTO opt_ins (uid, device_id, number, status, source) VALUES (?, ?, ?, 'pending', 'auto_invitation')",
                  [uid, deviceId, cleanNumber],
                );
              }
            }
            // ------------------------------

            await this.pool.query(
              "UPDATE messages SET status='failed', response=?, updated_at=? WHERE id=?",
              [check.reason, currentTime, id],
            );
            console.warn(
              `🚫 [Anti-Ban] Skip message ${id} to ${number} (Opt-In: ${check.reason})`,
            );
            continue;
          }
        }
      }

      // Validasi group
      if (msgType === "group" && !number.includes("@g.us")) {
        const [group] = await this.pool.query(
          "SELECT group_id FROM `groups` WHERE group_key=? LIMIT 1",
          [number],
        );
        if (!group.length) {
          await this.pool.query(
            "UPDATE messages SET status='failed', updated_at=? WHERE id=?",
            [currentTime, id],
          );
          failedConsecutive++;
          continue;
        }
        number = group[0].group_id;
      }

      try {
        // 1. Initial random interval (Human pause between chats)
        await this.delay(this.randomDelay(this.DELAY_BETWEEN_MESSAGES));

        // 2. Micro-sleep for extensive batching
        if (i > 0 && i % this.MESSAGES_BEFORE_MICRO_SLEEP === 0) {
          console.log(
            `💤 [Anti-Ban] Micro-sleep triggered for device ${deviceKey}...`,
          );
          await this.delay(this.randomDelay(this.MICRO_SLEEP));
        }

        // 3. Human-like Typing Simulation
        const realSession = this.sessionManager.getSession(deviceKey);
        if (realSession && realSession.socket) {
          try {
            // Start 'typing...'
            await realSession.socket.sendPresenceUpdate("composing", number);

            // Calculate typing duration based on message length (min 2s, max 10s)
            // Approx 100ms per character is a comfortable reading/typing speed simulation
            const typingDuration = Math.min(
              Math.max(message.length * 100, 2000),
              10000,
            );
            await this.delay(typingDuration);

            // Stop 'typing...' (optional, sending msg usually stops it, but good practice)
            await realSession.socket.sendPresenceUpdate("paused", number);
          } catch (presenceErr) {
            // Ignore presence errors, don't fail the message
            // console.warn('Presence update failed:', presenceErr.message);
          }
        }

        const resp = await this.messageManager.sendMessage(
          apiKey,
          deviceKey,
          number,
          message,
          msgType === "group" ? 1 : 0,
          tags,
        );
        const results = resp?.data?.results || [];
        const anySuccess = results.some((r) => r && r.status);
        const status = anySuccess ? "sent" : "failed";

        await this.pool.query(
          "UPDATE messages SET status=?, response=?, updated_at=? WHERE id=?",
          [status, JSON.stringify(resp), currentTime, id],
        );

        if (anySuccess) {
          failedConsecutive = 0;
          console.log(`✅ Sent message ${id} via ${deviceKey}`);
        } else {
          failedConsecutive++;
          console.warn(`⚠️ Failed sending message ${id} via ${deviceKey}`);
        }
      } catch (err) {
        console.error(`❌ Error sending ${id}:`, err.message);
        await this.pool.query(
          "UPDATE messages SET status='failed', updated_at=? WHERE id=?",
          [currentTime, id],
        );
        failedConsecutive++;
      }
    }
  }

  // ====================== DEVICE LIFETIME ======================

  async decrementDeviceLifeTime() {
    const today = moment().tz("Asia/Jakarta").format("YYYY-MM-DD");
    const now = moment().tz("Asia/Jakarta").format("YYYY-MM-DD HH:mm:ss");

    const sql = `
      UPDATE devices 
      SET life_time = life_time - 1,
          status = CASE WHEN life_time - 1 <= 0 THEN 'removed' ELSE status END,
          last_life_decrement=?, updated_at=?
      WHERE status='connected' AND life_time > 0 
        AND (last_life_decrement IS NULL OR last_life_decrement < ?)`;

    const [res] = await this.pool.query(sql, [today, now, today]);
    console.log(`🔁 Life_time decremented for ${res.affectedRows} devices.`);
  }

  // ====================== DEADLINE NOTIF ======================

  async generateDeadlineWarnings() {
    const sessions = await this.fetchActiveSessions(10, 0);
    for (const s of sessions) {
      const { deviceLifeTime, apiKey, deviceKey, devicePhone, deviceId } = s;
      let msg = "";

      switch (deviceLifeTime) {
        case 3:
          msg = `📢 Masa aktif perangkat *${deviceKey}* akan berakhir dalam 3 hari.`;
          break;
        case 2:
          msg = `⚠️ Masa aktif perangkat *${deviceKey}* tinggal 2 hari.`;
          break;
        case 1:
          msg = `⏰ Besok adalah hari terakhir masa aktif perangkat *${deviceKey}*.`;
          break;
        case 0:
          msg = `🚨 Masa aktif perangkat *${deviceKey}* berakhir hari ini.`;
          break;
      }
      if (!msg) continue;

      if (await this.hasSentWarningToday(deviceId)) continue;

      msg += `\n\n🔗 Perpanjangan: https://wapi.jasaedukasi.com/#pricing`;

      await this.messageManager.registerMessage(apiKey, deviceKey, {
        isGroup: 0,
        to: devicePhone,
        text: msg,
        tags: "Life Time",
      });
      console.log(
        `✅ Warning sent for ${deviceKey} (Life: ${deviceLifeTime} days)`,
      );
    }
  }

  async hasSentWarningToday(deviceId) {
    const [rows] = await this.pool.query(
      "SELECT COUNT(*) AS c FROM messages WHERE device_id=? AND tags='Life Time' AND DATE(created_at)=CURDATE()",
      [deviceId],
    );
    return rows[0].c > 0;
  }

  async getUserOptInConfig(uid, accessType = "owner") {
    const [rows] = await this.pool.query(
      "SELECT opt_in_required, template_invitation, template_invitation_shared FROM users WHERE uid = ? LIMIT 1",
      [uid],
    );
    const template =
      accessType === "shared" && rows[0]?.template_invitation_shared
        ? rows[0].template_invitation_shared
        : rows[0]?.template_invitation;
    const invitationText =
      template ||
      "Halo, ini adalah layanan notifikasi otomatis. Balas pesan ini dengan ketik 'SETUJU' untuk menerima informasi dan pembaruan dari kami otomatis melalui WhatsApp. Balas 'STOP' jika ingin berhenti berlangganan. Terima kasih!";

    return {
      required: Number(rows[0]?.opt_in_required || 0) === 1,
      invitationText,
      templateScope: accessType === "shared" && rows[0]?.template_invitation_shared
        ? "shared"
        : "default",
    };
  }

  async ensureOptInBeforeSending({
    uid,
    apiKey,
    deviceKey,
    deviceId,
    recipients,
    invitationText,
    currentTime,
    messageId,
  }) {
    const waitingRecipients = [];
    const blockedRecipients = [];

    for (const recipient of recipients) {
      const check = await this.messageManager.checkOptInStatus(
        uid,
        recipient,
        "",
        true,
      );
      if (check.allowed) continue;

      if (!check.reason.includes("belum melakukan Opt-In")) {
        blockedRecipients.push(recipient);
        continue;
      }

      waitingRecipients.push(recipient);
      const cleanNumber = recipient.replace(/\D/g, "");
      if (!cleanNumber) continue;

      const [optSearch] = await this.pool.query(
        "SELECT id, source FROM opt_ins WHERE uid = ? AND number = ? LIMIT 1",
        [uid, cleanNumber],
      );

      if (optSearch.length === 0) {
        console.log(`✉️ [Opt-In Required] Sending invitation to ${recipient}...`);
        await this.messageManager.sendMessage(
          apiKey,
          deviceKey,
          recipient,
          invitationText,
          0,
          "opt-in",
        );

        await this.pool.query(
          "INSERT INTO opt_ins (uid, device_id, number, status, source) VALUES (?, ?, ?, 'pending', 'auto_invitation')",
          [uid, deviceId, cleanNumber],
        );
      }
    }

    if (waitingRecipients.length > 0) {
      await this.pool.query(
        "UPDATE messages SET status='pending', response=?, updated_at=? WHERE id=?",
        [
          `Menunggu Opt-In: ${waitingRecipients.join(", ")}`,
          currentTime,
          messageId,
        ],
      );
    }

    return {
      waiting: waitingRecipients.length > 0,
      waitingRecipients,
      blocked: blockedRecipients.length > 0,
      blockedRecipients,
    };
  }

  // ====================== REMOVE SESSIONS ======================

  async removeRemovedDeviceSessions() {
    // Target devices that have been marked for final deletion by user/API
    const [rows] = await this.pool.query(
      "SELECT device_key FROM devices WHERE status='deleted'",
    );
    let count = 0;
    for (const r of rows) {
      const connection = await this.pool.getConnection();
      try {
        // Lookup device id first
        const [devRows] = await connection.query(
          "SELECT id FROM devices WHERE device_key = ? LIMIT 1",
          [r.device_key],
        );
        const deviceId = devRows.length ? devRows[0].id : null;

        // Remove session files first (best-effort)
        await this.sessionManager.removeSession(r.device_key, true);

        // Start DB transaction to remove related rows and device record atomically
        await connection.beginTransaction();

        // Delete messages for this device
        if (deviceId) {
          await connection.query("DELETE FROM messages WHERE device_id = ?", [
            deviceId,
          ]);
          await connection.query("DELETE FROM autoreply WHERE device_id = ?", [
            deviceId,
          ]);
        }

        // Groups use device_key
        await connection.query("DELETE FROM `groups` WHERE device_key = ?", [
          r.device_key,
        ]);

        // Finally delete device row
        await connection.query("DELETE FROM devices WHERE device_key = ?", [
          r.device_key,
        ]);

        await connection.commit();
        count++;
      } catch (err) {
        try {
          await connection.rollback();
        } catch (er) {
          /* ignore rollback error */
        }
        console.warn(
          `Failed to fully remove device ${r.device_key}:`,
          err && err.message,
        );
      } finally {
        connection.release();
      }
    }
    console.log(
      `🗑️ Removed ${count} sessions for 'deleted' devices (finalized user deletions).`,
    );
  }

  // ====================== TRANSACTION / PAYMENT CHECK ======================

  async checkPendingTransactions() {
    // expiry in minutes for pending invoices (default same as create-invoice expiry)
    const expiryMinutes = parseInt(
      process.env.DUITKU_EXPIRY_MINUTES || "10",
      10,
    );
    const now = moment().tz("Asia/Jakarta");
    const cutoff = now
      .clone()
      .subtract(expiryMinutes, "minutes")
      .format("YYYY-MM-DD HH:mm:ss");

    console.log(
      `🔍 Checking pending transactions older than ${expiryMinutes} minutes (cutoff=${cutoff})`,
    );

    try {
      const [rows] = await this.pool.query(
        `SELECT merchantOrderId, reference, amount, status, created_at FROM transactions WHERE status = 'pending' AND created_at <= ?`,
        [cutoff],
      );

      if (!rows.length) {
        console.log("✅ No expired pending transactions found.");
        return;
      }

      for (const tx of rows) {
        const { merchantOrderId } = tx;
        try {
          // Default behavior: mark as failed. If you want to query gateway first,
          // implement a call to Duitku inquiry API here and update accordingly.
          console.log(
            `⏳ Marking expired pending transaction ${merchantOrderId} -> failed`,
          );
          if (
            this.billingManager &&
            typeof this.billingManager.updateTransactionStatus === "function"
          ) {
            await this.billingManager.updateTransactionStatus(
              merchantOrderId,
              "failed",
            );
          } else {
            await this.pool.query(
              `UPDATE transactions SET status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE merchantOrderId = ?`,
              [merchantOrderId],
            );
          }
        } catch (err) {
          console.error(
            `❌ Failed to update transaction ${merchantOrderId}:`,
            err && err.message,
          );
        }
      }
    } catch (err) {
      console.error(
        "❌ Error checking pending transactions:",
        err && err.message,
      );
    }
  }

  // ====================== UTIL ======================

  randomDelay(range) {
    return range.min + Math.floor(Math.random() * (range.max - range.min));
  }

  delay(ms) {
    return new Promise((r) => setTimeout(r, ms));
  }
}

module.exports = CronManager;
