    ...
    async processMessagesByRole(role) {
        ...
        while (true) {
            ...
            for (const session of sessions) {
                await this.processMessages(session, role); // Tunggu setiap sesi selesai sebelum lanjut
                await new Promise((resolve) => setTimeout(resolve, 3000)); // Jeda 3 detik antar sesi
            }
    
            offset += limitSessions;
            await new Promise((resolve) => setTimeout(resolve, 5000)); // Jeda 5 detik antar batch sesi
        }
    }
    ...
    async processMessages(session, role) {
        ...
        try {
            ...
            for (const row of rows) {
                ...
                // Jeda 1.5 detik antar pesan agar tidak dianggap spam
