...
class CronManager {
    ...
    async deleteOldMessages() {
        ...
        const deleteSentTime = now.subtract(1, 'months').format('YYYY-MM-DD HH:mm:ss');  // 1 bulan
        const deleteOtherTime = now.subtract(2, 'months').format('YYYY-MM-DD HH:mm:ss'); // 2 bulan
        ...
        try {
            ...
            console.log(`Deleted ${sentResult.affectedRows} messages with status 'sent' older than 1 month.`);
            ...
            console.log(`Deleted ${otherResult.affectedRows} messages with status 'pending', 'failed', or 'processing' older than 2 months.`);
        ...
        try {
            ...
            for (const row of rows) {
                ...
                try {
                    ...
                    const response = await this.messageManager.sendMessage(apiKey, deviceKey, number, message, type === 'group' ? 1 : 0);

                    let status = 'failed';
                    if (response.status) {
                        const allSentSuccessfully = response.data.results.every(result => result.status === true);
                        status = allSentSuccessfully ? 'sent' : 'failed';
                    }
                    const updateSql = `
                        UPDATE messages 
