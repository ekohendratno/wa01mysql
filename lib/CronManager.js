...
class CronManager {
  ...
  async initCrons() {
    ...
    // ... (separated by other code, I will use separate chunks if needed but view shows they are far apart. Wait, t3 is lines 81-86. deleteOldMessages is lines 134-164. I should use MultiReplace or multiple Replace calls. I'll use MultiReplace.)
  ...
  async processMessages(session, type) {
    ...
    const [dailyCount] = await this.pool.query(
      `SELECT COUNT(*) AS sentToday FROM messages WHERE device_id = ? AND DATE(created_at) = ? AND status IN ('sent', 'failed', 'processing')`,
      [deviceId, today],
    );
